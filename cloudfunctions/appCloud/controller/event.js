...
module.exports = async (event, context) => {
    ...
    if (method === 'ORDER_PAY_SUCCESS') {
        ...
        // let attach = JSON.parse(decodeURIComponent(orderRes.order.attach) || '{}') || {};
        const curMemberType = orderRes.order.memberType;
        let good = GOODS.find(item => String(item.memberType) === String(curMemberType));
        console.log('神奇的memberType', curMemberType, good);
        const curMemberDay = good.memberDay;
        // if (!attach.memberDay) {
        ...
        console.log('第四步', fromOpenidRes, userRes);
        if (fromOpenidRes) {
            const cash = appConfig.awardPlan.find(i => String(i.memberType) === String(curMemberType)).cash;
